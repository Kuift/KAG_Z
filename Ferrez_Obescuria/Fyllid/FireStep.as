...
void onCollision(CBlob@ this, CBlob@ blob, bool solid, Vec2f normal, Vec2f point1) {
	if (blob !is null && blob.getShape().getConsts().isFlammable) {
		...
		this.server_Hit(blob, point1, Vec2f(0,0), 0.01, Hitters_modfire);
	}

	Vec2f lastBurnPos = this.get_Vec2f("last burn pos");
	getMap().server_setFireWorldspace(lastBurnPos, false);
	getMap().server_setFireWorldspace(this.getOldPosition(), true);
	this.set_Vec2f("last burn pos", this.getOldPosition());
}
