...
void onTick(CMovement@ this)
{
	...
	if (brain !is null)
	{
		...
		if (target !is null)
		{
			if ((target.getPosition() - blob.getPosition()).getLength() < blob.getRadius() * 1.0f && target.getPosition().y < blob.getPosition().y - blob.getRadius())
			{
				up = true;
			}
		}
	...
	if (left || right || up)
	{
		...
		if ((blob.isOnGround() || blob.isInWater()) && (up || (right && map.isTileSolid(Vec2f(pos.x + radius, pos.y + 1.00f * radius))) || (left && map.isTileSolid(Vec2f(pos.x - radius, pos.y + 1.00f * radius)))
		                                               )
		   )
		{
