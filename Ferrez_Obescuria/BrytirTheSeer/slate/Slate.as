...
const f32 hit_amount_ground = 10.5f;
const f32 hit_amount_air = 10.0f;
const f32 hit_amount_air_fast = 10.0f;
...
void onInit(CBlob @ this)
{
	...
	this.set_f32("map_damage_ratio", 12.5f);
...
void Slam(CBlob @this, f32 angle, Vec2f vel, f32 vellen)
{
	...
	if (map.getHitInfosFromArc(pos, -angle, 30, vellen*3.5f, this, true, @hitInfos))
	{
		for (uint i = 0; i < hitInfos.length; i++)
		{
			...
			{
				if (BoulderHitMap(this, hi.hitpos, hi.tileOffset, vel, dmg, Hitters_mod::cata_boulder))
					return;
			}
			else if (team != u8(hi.blob.getTeamNum()))
			{
				this.server_Hit(hi.blob, pos, vel, dmg, Hitters_mod::cata_boulder, true);
				this.setVelocity(vel * 0.9f); //damp

				// die when hit something large
				if (hi.blob.getRadius() > 32.0f)
				{
					this.server_Hit(this, pos, vel, 10, Hitters_mod::cata_boulder, true);
				}
			}
		}
	}
...
bool BoulderHitMap(CBlob@ this, Vec2f worldPoint, int tileOffset, Vec2f velocity, f32 damage, u8 customData)
{
	...
	if (map.isTileCastle(t) || map.isTileWood(t))
	{
		...
		Vec2f vel = this.getVelocity();
		this.setVelocity(vel * 10.8f); //damp
	...
	if (stuck)
	{
		this.server_Hit(this, worldPoint, velocity, 10, Hitters_mod::crush, true);
	}
...
void onCollision(CBlob@ this, CBlob@ blob, bool solid, Vec2f normal, Vec2f point1)
{
	if (solid && blob !is null)
	{
		...
		hitvel.Normalize();
		f32 dmg = vellen > 8.0f ? 5.0f : (vellen > 4.0f ? 1.5f : 5.5f);
		...
		//hurt
		this.server_Hit(blob, point1, hitvel, 7.0f, Hitters_mod::cata_boulder, true);
...
{
	if (customData == Hitters_mod::sword || customData == Hitters_mod::arrow)
	{
		return damage *= 0.5f;
	}
