...
void onTick(CBlob@ this)
{
	...
	{
		this.server_Hit(this, pos, Vec2f(0, -1), 3.0f, Hitters_modfall, true);
		return;
	}
				...
				if (map.getBlobsInRadius(pos, this.getRadius() * 2.0f, @blobsInRadius))
				{
					for (uint i = 0; i < blobsInRadius.length; i++)
					{
						...
						if (canStab(b)) //even hurts team when stabbing
						{
							...
							{
								this.server_Hit(b, pos, b.getVelocity() * -1, 0.5f, Hitters_modspikes, true);
							}
						}
					}
				}
...
void onCollision(CBlob@ this, CBlob@ blob, bool solid, Vec2f normal, Vec2f point)
{
	...
	if (state == falling)
	{
		...
		if (vellen < 4.0f){
            this.server_Hit(blob, point, Vec2f(0, 1), 2.0f, Hitters_modspikes, true);
        }
        else if(vellen < 5.5f){
            this.server_Hit(blob, point, Vec2f(0, 1), 4.0f, Hitters_modspikes, true);
        }
        else if(vellen < 7.0f){
            this.server_Hit(blob, point, Vec2f(0, 1), 6.0f, Hitters_modspikes, true);
        }
        else if(vellen < 8.5f){
            this.server_Hit(blob, point, Vec2f(0, 1), 8.0f, Hitters_modspikes, true);
        }
        else if(vellen < 10.0f){
            this.server_Hit(blob, point, Vec2f(0, 1), 10.0f, Hitters_modspikes, true);
        }
	}
	...
	if (damage > 0)
	{
		this.server_Hit(blob, point, vel * -1, damage, Hitters_modspikes, true);
	}
}
