				...
				else
				{
					...
					if (!inair)
					{
						...
						if (knight.slideTime > 0)
						{
							...
							f32 factor = Maths::Max(1.0f, 2.2f / Maths::Sqrt(knight.slideTime)) *0.9f;
	...
	else if ((pressed_a1 || swordState) && !moveVars.wallsliding)   //no attacking during a slide
	{
		         ...
		         knight.state <= KnightStates::sword_cut_down) // cut state
		{
			...
			if (delta > DELTA_BEGIN_ATTACK && delta < DELTA_END_ATTACK)
			{
				...
				DoAttack(this, 2.0f, attackAngle, attackarc, Hitters_mod::sword, delta, knight);
			}
			else if (delta >= 9)
			{
				knight.swordTimer = 0;
		         ...
		         knight.state == KnightStates::sword_power_super)
		{
			...
			{
				DoAttack(this, 4.0f, -(vec.Angle()), 80.0f, Hitters_mod::sword, delta, knight);
			}
		        ...
		        delta < KnightVars::slash_move_time)
		{
			...
			{
				Vec2f slash_vel =  knight.slash_direction * this.getMass() * 1.1f;
