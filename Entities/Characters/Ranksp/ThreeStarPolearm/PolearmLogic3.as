...
this.push("names to activate", "keg");
	...
	addShieldVars(this, SHIELD_BLOCK_ANGLE, 1.0f, 2.5f);
				...
				else
				{
					...
					if (!inair)
					{
						...
						if (knight.slideTime > 0)
						{
							...
							f32 factor = Maths::Max(1.0f, 2.2f / Maths::Sqrt(knight.slideTime)) *0.8f;
	...
	else if ((pressed_a1 || swordState) && !moveVars.wallsliding)   //no attacking during a slide
	{
		         ...
		         knight.state <= KnightStates::sword_cut_down) // cut state
		{
			...
			if (delta > DELTA_BEGIN_ATTACK && delta < DELTA_END_ATTACK)
			{
				...
				DoAttack(this, 3.5f, attackAngle, attackarc, Hitters_mod::sword, delta, knight);
			}
			else if (delta >= 9)
			{
				knight.swordTimer = 0;
		         ...
		         knight.state == KnightStates::sword_power_super)
		{
			...
			{
				DoAttack(this, 7.0f, -(vec.Angle()), 80.0f, Hitters_mod::sword, delta, knight);
			}
		        ...
		        delta < KnightVars::slash_move_time)
		{
			...
			{
				Vec2f slash_vel =  knight.slash_direction * this.getMass() * 1.2f;
			...
			else if (bombType == 1)
			{
				if (getNet().isServer())
				{
					...
					if (blob !is null)
					{
						...
						blob.set_u8("custom_hitter", Hitters_mod::water);
...
void DoAttack(CBlob@ this, f32 damage, f32 aimangle, f32 arcdegrees, u8 type, int deltaInt, PolearmInfo@ info)
{
	if (!getNet().isServer())
	{
		return;
...
bool isSliding(PolearmInfo@ knight)
{
		        ...
		        getKnocked(blob) == 0)
		{
			...
			if (vel * normal < 0.0f && knight_hit_actor_count(this) == 0) //only bash one thing per tick
			{
				...
				if (shieldVars.direction * normal < 0.0f)
				{
					...
					this.server_Hit(blob, pos, vel, 0.0f, Hitters_mod::shield);
...
void onHitBlob(CBlob@ this, Vec2f worldPoint, Vec2f velocity, f32 damage, CBlob@ hitBlob, u8 customData)
{
	...
	if (customData == Hitters_mod::shield)
	{
