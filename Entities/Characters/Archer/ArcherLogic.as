...
const f32 CHARGING_UPGRADE_RATE = 0.017f; //amount of charging speed increase per level
const f32 MOBILITY_UPGRADE_RATE = 0.016f; //amount of grapple velocity and lenght increased per level
...
void ManageGrapple(CBlob@ this, ArcherInfo@ archer)
{
	...
	if (right_click && charge_state != ArcherParams::stabbing)
	{
		...
		else if (canSend(this)) //otherwise grapple
		{
			...
			if (distance > 1.0f)
			{
				archer.grapple_vel = direction * archer_grapple_throw_speed * (1.0f + getRules().get_u16("archer level") * MOBILITY_UPGRADE_RATE);
			}
			else
			{
	...
	if (archer.grappling)
	{
		...
		{
			const f32 archer_grapple_range = archer_grapple_length * archer.grapple_ratio * (1.0f + getRules().get_u16("archer level") * MOBILITY_UPGRADE_RATE/3.0f);
			const f32 archer_grapple_force_limit = this.getMass() * archer_grapple_accel_limit * (1.0f + getRules().get_u16("archer level") * MOBILITY_UPGRADE_RATE*3.3f);
...
void ManageBow(CBlob@ this, ArcherInfo@ archer, RunnerMoveVars@ moveVars)
{
	...
	else if (this.isKeyPressed(key_action1))
	{
		...
		{
			charge_time += 1.0f + getRules().get_u16("archer level")*CHARGING_UPGRADE_RATE;
			...
			else if(charge_state == ArcherParams::stabbing)
			{
				...
				if (archer.stab_delay == STAB_DELAY)
				{
					...
					if (stabTarget !is null)
					{
						...
						{
							this.server_Hit(stabTarget, stabTarget.getPosition(), Vec2f_zero, 0.25f,  Hitters_modstab);
