...
void onInit(CBlob@ this)
{
	...
	knight.tripleslash = false;
	knight.triggerslash = false;
...
void onTick(CBlob@ this)
{
	...
	else if ((pressed_a1 || swordState) && !moveVars.wallsliding)   //no attacking during a slide
	{
		         ...
		         knight.state == KnightStates::sword_power_super)
		{
			...
			{
				if (knight.tripleslash)
				{
				knight.triggerslash = true;
				}
				knight.swordTimer = 0;
	...
	if (cmd == this.getCommandID("get bomb"))
	{
		...
		if (hasItem(this, bombTypeName))
		{
			if (bombType == 0)
			{
				if (getNet().isServer())
				{
					CBlob @blob = server_CreateBlob("clusterbomb", this.getTeamNum(), this.getPosition());
					if (blob !is null)
					{
