...
void onTick(CBlob@ this)
{
  ...
  bool ready = this.get_bool("shok ready");
	...
	if (this.getMap().getBlobsInRadius(this.getPosition(), max_range, @blobs) && this.hasTag("shok"))
	{
		...
		{
			CBlob@ blob = blobs[i];
			if(!blob.hasTag("zombie") && !blob.hasTag("portal_zombie") && !blob.hasTag("ZombiePortalz") && !blob.hasTag("player") && !blob.hasTag("fanatic"))
			{
				continue;
			}
			...
			if (!this.getMap().rayCastSolidNoBlobs(blob.getPosition(), this.getPosition()))
			{
				...
				{
				this.server_Hit(blob, blob.getPosition(), Vec2f(0,0), (1.5f*XORRandom(8)/3)*XORRandom(4), Hitters_mod::arc);
