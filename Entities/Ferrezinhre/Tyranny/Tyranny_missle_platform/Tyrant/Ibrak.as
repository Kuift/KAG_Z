	...
	if (speed > 0.1f && map.getHitInfosFromArc(tail_position, -angle, 10, (tip_position - tail_position).getLength(), this, true, @infos))
	{
		...
		for (uint i = 0; i < infos.length; i ++)
		{
			...
			if (blob !is null)
			{
				...
				this.server_Hit(blob, hit_position, velocity, 2.5f, Hitters_modballista, true);
				BallistaHitBlob(this, hit_position, velocity, 2.5f, blob, Hitters_modballista);
...
bool DoExplosion(CBlob@ this, Vec2f velocity)
{
	...
	if (this.hasTag("bomb"))
	{
		...
		LinearExplosion(this, velocity, 12.0f, 8.0f, 2, 4.0f, Hitters_modblast);
