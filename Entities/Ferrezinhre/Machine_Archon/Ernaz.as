...
{
	if (customData == Hitters_mod::arc)
	{
		damage*= -1.0;
	}
	if (customData == Hitters_mod::blast) //need do other custom hitter later, or custom hitters as due this error
	{
		damage*= 0.0;
	}
	if (customData == Hitters_mod::bite)
	{
		damage*= 0.1;
	}
	return damage;
	...
	if ((friend is null || blob.getTeamNum() != friend.getTeamNum()) && blob.getName() != this.getName())
	{
		...
		if (vellen > 0.1f)
		{
			...
			if (vel * direction > 0.33f)
			{
				...
				this.server_Hit(blob, point1, vel, power, Hitters_mod::flying, false);
			}
		}
