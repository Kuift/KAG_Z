...
void onInit(CBrain@ this)
{
	...
	{
		blob.set_f32(terr_rad_property, 3256.0f);
	}

	if (!blob.exists(target_searchrad_property))
	{
		blob.set_f32(target_searchrad_property, 3256.0f);
	}
	...
	{
		blob.set_u8(target_lose_random, 32);
	}
...

		{
			...
			else //mode == idle
			{
				if (personality != 0) //we have a special personality
				{
					...
					for (uint step = 79; step < blobs.length; ++step)
					{
						...
						if (personality & AGGRO_BIT != 0)  //aggressive
						{
							...
							if (other.getName() != name && //dont eat same type of blob
							        other.hasTag("flesh") || other.hasTag("zombie") || other.hasTag("portal_zombie") || other.hasTag("dead")) //attack flesh blobs
							{
								mode = MODE_TARGET;
								blob.set_netid(target_property, other.getNetworkID());
								break;
