 ...
 *
 * u8 custom_hitter             - the hitter from Hitters_mod.as to use
 */
...
void Explode ( CBlob@ this, f32 radius, f32 damage )
{
    ...
    //actor damage
    u8 hitter = Hitters_mod::explosion;
                            ...
                            if (canHit)
                            {
								...
								if (map.getSectorAtPosition( tpos, "no build") is null)
								{
									...
									if (!map.isTileBedrock(tile))
									{
										...
										if (dist >= rad_thresh || (hitter == Hitters_mod::bomb && map.isTileGroundStuff(tile ))  ) 										{
...
{			 
    if (customData == Hitters_mod::bomb || customData == Hitters_mod::water)
    {
        hitBlob.AddForce( velocity );
    }
	
	if (((XORRandom(10.0) - (XORRandom(10.0))) > 1.0))
        {
			CBlob @misery = server_CreateBlob( "misery", this.getTeamNum(), this.getPosition()+ Vec2f(0 , 0.0f - XORRandom(160)));
			return;
        }
}	  
			  ...
			  const bool bother_raycasting = true, const bool should_teamkill = false )
{
	...
	this.server_Hit( 	hit_blob, hit_blob_pos,
						bombforce, dam,
						hitter, hitter == Hitters_mod::water || //hit with water
