...
{
	string config_filename = this.get_string("gununmaker_config");
	if(config_filename == "") {
...
void onCommand(CBlob@ this, u8 cmd, CBitStream @params)
{
   if (cmd == this.getCommandID("shoot"))
   {
	...
	f32 speedmin = this.get_f32("gun_speedmin");
	 ...
	 for (int i = 0; i < 7; ++i) // tweak for the shotgun effect by Asu
	 {
       ...
       CBlob@ bullet = server_CreateBlobNoInit("glyphenergybolt");
       if (bullet !is null)
       {
         float cangle = angle;
         ...
         {
           offset.RotateBy(cangle, Vec2f(-5, 0));
