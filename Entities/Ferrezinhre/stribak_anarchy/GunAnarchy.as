...
{
	string config_filename = this.get_string("gunanarchy_config");
	if(config_filename == "") {
...
void onCommand(CBlob@ this, u8 cmd, CBitStream @params)
{
   if (cmd == this.getCommandID("shoot"))
   {
	...
	f32 speedmin = this.get_f32("gun_speedmin");
	 ...
	 for (int i = 0; i < 3; ++i) // tweak for the shotgun effect by Asu
	 {
       ...
       CBlob@ bullet = server_CreateBlobNoInit("anarchyammo0") ;
       if (bullet !is null)
       {
         float cangle = angle;
