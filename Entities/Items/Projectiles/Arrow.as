...
const f32 ARROW_DAMAGE_UPGRADE_RATE = 0.1f; // damage increase per archer level
//Arrow logic
...
void onTick(CBlob@ this)
{
	...
	if (!this.hasTag("collided")) //we haven't hit anything yet!
	{
		...
		if (this.hasTag("shotgunned"))
		{
			...
			{
				this.server_Hit(this, this.getPosition(), Vec2f(), 1.0f, Hitters_mod::crush);
			}
		}
...
void onCollision(CBlob@ this, CBlob@ blob, bool solid, Vec2f normal, Vec2f point1)
{
	if (blob !is null && doesCollideWithBlob(this, blob) && !this.hasTag("collided"))
	{
		...
		{
			//apply a hard hit
			dmg = 1.5f + getRules().get_u16("archer level")*ARROW_DAMAGE_UPGRADE_RATE*2;
		}
		else
		{
		...
		if (dmg > 0.0f)
		{
			...
			const u8 hit_type =
				(arrowType == ArrowType::bomb) ? Hitters_mod::bomb_arrow :
				Hitters_mod::arrow;
			...
			if (arrowType == ArrowType::fire && !this.hasTag("no_fire"))
				this.server_Hit(blob, point1, initVelocity, 0.0f, Hitters_mod::fire);
...
f32 ArrowHitBlob(CBlob@ this, Vec2f worldPoint, Vec2f velocity, f32 damage, CBlob@ hitBlob, u8 customData, const u8 arrowType)
{
	if (hitBlob !is null)
	{
		...
		{
			ArrowHitMap(this, worldPoint, velocity, damage, Hitters_mod::arrow);
		}
		//die otherwise
		else
		{
...
void ArrowHitMap(CBlob@ this, Vec2f worldPoint, Vec2f velocity, f32 damage, u8 customData)
{
	...
	if (this.getMap().getBlobsInRadius(worldPoint, this.getRadius() * 1.3f, @blobsInRadius))
	{
		for (uint i = 0; i < blobsInRadius.length; i++)
		{
			...
			{
				this.server_Hit(b, worldPoint, Vec2f(0, 0), velocity.Length() / 7.0f, Hitters_mod::arrow);
				break;
			}
		}
	}
...
void MakeFireCross(CBlob@ this, Vec2f burnpos)
{
	...
	*/
	...
	for (int i = 0; i < positions.length; i++)
	{
		...
		{
			this.server_Hit(b, this.getPosition(), this.getVelocity(), 0.5f, Hitters_mod::fire);
		}
	}
}
...
f32 getArrowDamage(CBlob@ this, f32 vellen = -1.0f)
{
	...
	{
		return 1.0f + getRules().get_u16("archer level")*ARROW_DAMAGE_UPGRADE_RATE;
	}
	else if (vellen >= arrowMediumSpeed)
	{
		return 1.0f + getRules().get_u16("archer level")*ARROW_DAMAGE_UPGRADE_RATE*0.66;
	}

	return 0.5f + getRules().get_u16("archer level")*ARROW_DAMAGE_UPGRADE_RATE*0.66;
}
