...
void onCommand(CBlob@ this, u8 cmd, CBitStream @params)
{
	...
	else if (cmd == this.getCommandID("getout"))
	{
		...
		// this.server_Hit(this, this.getPosition(), Vec2f(), 100.0f, Hitters_mod::crush, true);
		// this.server_Die();
	}
...
void onRemoveFromInventory(CBlob@ this, CBlob@ blob)
{
	...
	if (blob.getName() == "keg")
	{
		...
		{
			this.server_Hit(this, this.getPosition(), Vec2f(), 100.0f, Hitters_mod::explosion, true);
		}
...
f32 onHit( CBlob@ this, Vec2f worldPoint, Vec2f velocity, f32 damage, CBlob@ hitterBlob, u8 customData )
{
	...
	if (customData == Hitters_mod::builder)
	{
		dmg *= 4;
	}
	if (customData == Hitters_mod::saw)
	{
		DumpOutItems(this, 0);
	}
	if (isExplosionHitter(customData) || customData == Hitters_mod::keg)
	{
		if (dmg > 50.0f) // inventory explosion
		{
			...
			if (sneaky_player !is null)
			{
				hitterBlob.server_Hit(sneaky_player, this.getPosition(), Vec2f(),
									  sneaky_player.getInitialHealth() * 2 - 0.25f, Hitters_mod::explosion, true);
			}
		}
		else
		{
			if (customData == Hitters_mod::keg)
			{
