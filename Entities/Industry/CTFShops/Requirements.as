...
string getButtonRequirementsText(CBitStream& inout bs, bool missing)
{
	...
	while (!bs.isBufferEnd())
	{
		...
		else if (requiredType == "not tech no extra text" && missing)
		{
			text += " \n";
			text += quantityColor;
			text += friendlyName;
...
void server_TakeRequirements(CInventory@ inv1, CInventory@ inv2, CBitStream &inout bs)
{
	...
	while (!bs.isBufferEnd())
	{
		...
		else if (req == "coingive") // TODO...
		{
			...
			if (player1 !is null)
			{
				player1.server_setCoins(player1.getCoins() + quantity);
			}
			if (player2 !is null)
			{
				player2.server_setCoins(player2.getCoins() + quantity);
			}
		}
	}
