...
void onTick(CBlob@ this)
{
	...
	if (this.isAttached())
	{
		...
		if (int(heat) >= heat_max - (heat_add * 1.5))
		{
			...
			this.server_Hit(holder, holder.getPosition(), Vec2f(), 0.25f, Hitters_modburn, true);
			this.server_DetachFrom(holder);
			sprite.PlaySound("DrillOverheat.ogg");
		}

		if (holder.getName() == "builder" || holder.getName() == "onestarbuilder" || holder.getName() == "twostarbuilder" || holder.getName() == "threestarbuilder")
		{
						...
						for (uint i = 0; i < hitInfos.length; i++)
						{
							...
							if (b !is null) // blob
							{
								...
								if (isServer())
								{
									...
									this.server_Hit(b, hi.hitpos, attackVel, attack_dam, Hitters_moddrill);
...
{
	if (customData == Hitters_modfire)
	{
		this.set_u8(heat_prop, heat_max);
		makeSteamPuff(this);
	}

	if (customData == Hitters_modwater)
	{
...
void onRender(CSprite@ this)
{
	...
	if (holderBlob.getName() != "builder" || holderBlob.getName() != "onestarbuilder" || holderBlob.getName() != "twostarbuilder" || holderBlob.getName() != "threestarbuilder" && sv_gamemode != "TDM"){return;}
