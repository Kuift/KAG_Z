						...
						for (uint i = 0; i < hitInfos.length; i++)
						{
							...
							if (other !is null)
							{
								...
								else if (!hit_block)
								{
									...
									{
										this.server_Hit(other,other.getPosition(),vel,3.0,Hitters::saw, false);
										this.set_u16("lastbite",0);
										hit_block=true;
									}
...
void onDie( CBlob@ this )
{
	if (!getNet().isServer()) return;
					...
					if (r == 0)
					{
						if ((XORRandom(1024) / 1024.0f) < 0.1)
						server_CreateBlob("scarnage",this.getTeamNum(), this.getPosition());
					}
					else if (r == 1)
					{
					    if ((XORRandom(1024) / 1024.0f) < 0.1)
						server_CreateBlob("smidas",this.getTeamNum(), this.getPosition());
					}
					else if (r == 2)
					{
						if ((XORRandom(1024) / 1024.0f) < 0.1)
						server_CreateBlob("staming",this.getTeamNum(), this.getPosition());
					}
					else if (r == 3)
					{
						if ((XORRandom(1024) / 1024.0f) < 0.3)
						server_CreateBlob("sheal",this.getTeamNum(), this.getPosition());
					}
					else if (r == 4)
					{
					    if ((XORRandom(1024) / 1024.0f) < 0.5)
						server_DropCoins(this.getPosition() + Vec2f(0,-3.0f), 100);
					}
